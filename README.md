...
Please visit following link for installation steps of GCM  
...
* This implementation is optional, If application have requirement to convey payment status of pending transactions to user. Just remove mRegistrationBroadcastReceiver from CartActivity from example 
 
##Show Notifications  
When any pending payment completed we received push from server,  
To handle this notification there is one class created named **NotificationUtils**.  
In this class there is one function  
```  
public static void sendNotification(Context context, Bundle data)  
```  
This function takes 2 params  
1.context - We can provide activity context or application context.  
2.data - data is a bundle which contain all the required data about payment.  
Rest will be handled by function. It will open notification page on click of notification.
